...
def build_roboflow(image_set, args, resolution):
    ...
    # --- START OF THE FIX ---
    # Map the generic 'val' split name to the actual 'valid' folder name.
    image_set_map = {'train': 'train', 'val': 'valid', 'test': 'test'}
    image_set_folder_name = image_set_map.get(image_set.split("_")[0], image_set.split("_")[0])

    # Construct the correct paths based on your file structure.
    img_folder = root / image_set_folder_name / "images"
    ann_file = root / "coco_annotations" / f"{image_set_folder_name}.json"
    # --- END OF THE FIX ---
