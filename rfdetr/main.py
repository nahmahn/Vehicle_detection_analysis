...
class Model:
        ...
        if args.pretrain_weights is not None:
            ...
            if args.pretrain_keys_modify_to_load is not None:
                ...
                for modify_key_to_load in args.pretrain_keys_modify_to_load:
                    try:
                        # Use self.model, which is defined in this scope
                        ...
                        print(f"Failed to modify key {modify_key_to_load}: {e}, deleting from checkpoint")
                        checkpoint['model'].pop(modify_key_to_load, None)
    ...
    def train(self, callbacks: DefaultDict[str, List[Callable]], **kwargs):
        ...
        # Initialize all test-related variables to None
        dataset_test = None
        base_ds_test = None
        data_loader_test = None
        
        # Define the expected path for the test annotation file
        # This logic must match the path construction in `datasets/coco.py`
        test_ann_path = Path(args.dataset_dir) / "coco_annotations" / "test.json"
        
        if test_ann_path.exists():
            print("Test set annotation found. Building test dataset...")
            ...
            print(f"WARNING: Test set annotation not found at '{test_ann_path}'. Skipping test set operations.")
        ...
        if args.distributed:
            ...
            if dataset_test: sampler_test = torch.utils.data.SequentialSampler(dataset_test)
        else:
        ...
        if args.run_test:
            if data_loader_test and base_ds_test:
                print("\nRunning final evaluation on the test set...")
                best_is_ema = best_map_holder.is_ema_best()
                model_to_test = self.ema_m.module if best_is_ema else model_without_ddp
                test_stats, _ = evaluate(model_to_test, criterion, postprocessors, data_loader_test, base_ds_test, device, args=args)
                if utils.is_main_process():
                    ...
                    with open(output_dir / "results.json", "w") as f:
                        json.dump(results, f)
                    print('Test results saved to {}'.format(output_dir / "results.json"))
            else:
                print("\nSkipping final evaluation on test set because it was not loaded.")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('RF-DETR Command-Line Utility', parents=[get_args_parser()])
