...
def get_norm(norm, out_channels):
    ...
    if isinstance(norm, str):
        if len(norm) == 0: return None
        norm = {"LN": lambda channels: LayerNorm(channels)}[norm]
    return norm(out_channels)

def get_activation(name, inplace=False):
    if name == "silu": return nn.SiLU(inplace=inplace)
    if name == "relu": return nn.ReLU(inplace=inplace)
    if name in ["LeakyReLU", 'leakyrelu', 'lrelu']: return nn.LeakyReLU(0.1, inplace=inplace)
...
class ConvX(nn.Module):
    def __init__(self, in_planes, out_planes, kernel=3, stride=1, groups=1, dilation=1, act='relu', layer_norm=False, rms_norm=False):
        super(ConvX, self).__init__()
        if not isinstance(kernel, tuple): kernel = (kernel, kernel)
        padding = (kernel[0] // 2, kernel[1] // 2)
        self.conv = nn.Conv2d(in_planes, out_planes, kernel_size=kernel, stride=stride, padding=padding, groups=groups, dilation=dilation, bias=False)
        if rms_norm: self.bn = nn.RMSNorm(out_planes)
        else: self.bn = get_norm('LN', out_planes) if layer_norm else nn.BatchNorm2d(out_planes)
        self.act = get_activation(act, inplace=True)
...
class MultiScaleProjector(nn.Module):
    def __init__(
        ...
        # Maps scale name (e.g. 'P4') to the index of the feature in the input list `x`
        # Assumes the backbone provides features in ascending order of scale name (P3, P4, P5...)
        self.scale_to_index = {f'P{i+3}': i for i in range(len(in_channels))}
        self.use_extra_pool = "P6" in scale_names

        self.stages = nn.ModuleList()

        # Build a C2f processing stage for each required feature scale
        for scale_name in self.scale_names:
            if scale_name == 'P6': continue # P6 is special, handled by max pooling

            if scale_name not in self.scale_to_index:
                 raise ValueError(f"Scale '{scale_name}' not found. Available from backbone: {list(self.scale_to_index.keys())}")

            idx = self.scale_to_index[scale_name]
            current_in_channels = in_channels[idx]

            stage = nn.Sequential(
                C2f(current_in_channels, out_channels, num_blocks, layer_norm=layer_norm),
                get_norm('LN', out_channels)
            )
    ...
    def forward(self, x: list):
        ...
        stage_idx = 0
        for scale_name in self.scale_names:
            if scale_name == 'P6': continue

            feature_index = self.scale_to_index[scale_name]
            if feature_index >= len(x):
                raise IndexError(f"Trying to access feature at index {feature_index}, but backbone only provided {len(x)} features.")

            processed_feat = self.stages[stage_idx](x[feature_index])
            results.append(processed_feat)
            stage_idx += 1

        if self.use_extra_pool:
            if not results:
                raise ValueError("Cannot create P6 feature because no other features were processed.")
            p6_feat = F.max_pool2d(results[-1], kernel_size=1, stride=2, padding=0)
...
class C2f(nn.Module):
    ...
    def forward(self, x):
        """Forward pass using explicit slicing to avoid non-contiguous memory issues on MPS."""
        x_cv1 = self.cv1(x)

        # Splot the tensor using explicit slicing
        y1 = x_cv1[:, :self.c, :, :]
        y2 = x_cv1[:, self.c:, :, :]
        y = [y1, y2.contiguous()]
        y.extend(m(y[-1]) for m in self.m)
