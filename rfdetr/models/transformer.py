...
class Transformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, refpoint_embed, query_feat):
        if not srcs:
            raise ValueError(
                "Transformer received an empty list of source features (srcs). "
                "This is likely due to a misconfiguration in the backbone or projector "
                "which resulted in no feature maps being passed to the transformer."
            )
        src_flatten = []
